...
namespace BasicDriverUnitTest
{
    TEST_CLASS(BasicDriverFunctionTest)
    {
        ...
        TEST_METHOD(MockPrivateFunction)
        {
            ...
            LONG v = testfunc(10);
            Assert::AreEqual(111L, v);
        }

        // Мок с фиксированным возвратом (полная замена логики функции)
        TEST_METHOD(MyFixedReturnMock)
        {
            TEST_DEFINE_MOCK_FUNCTION(fixedMock, LONG, "PrivateTestFunction", (LONG v)) {
                return 99L;
            } TEST_END_MOCK;

            TEST_USING_MOCK(fixedMock);

            LONG result = fixedMock(42);
